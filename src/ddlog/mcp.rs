...
impl ProgramInstance {
    ...
    fn call(&mut self, name: &str, a: &Value) -> Result<Value, String> {
        if name == "instance_info" && self.instance_id.is_some() {
            return Ok(
                json!({"instance_id":self.instance_id,"health":self.backend.health(),"processor":self.processor}),
            );
        }
        ...
        match name {
            "processor_install" => {
                ...
                let mut result = if let Some(binding) = definition.operation {
                    let operation = self
                        .operations
                        .get(&binding.name)
            ...
            "lemmalog_query" => self.backend.query(string(a, "predicate")?),
            "lemmalog_why" => self.backend.why(
...
fn registry_tools() -> Value {
    let definition = json!({"type":"object","properties":{"rules":{"type":"string"},"schemas":{"type":"object"},"operation":{"type":["object","null"],"properties":{"name":{"type":"string"},"version":{"type":"string"},"description":{"type":"string"}},"required":["name","version","description"],"additionalProperties":false}},"required":["rules","schemas"],"additionalProperties":false});
    ...
    json!([
        tool("processor_create","Validate syntax and types, then save an immutable processor definition under a new stable identity. Does not compile or activate a graph.",json!({"definition":definition,"git_provenance":{"type":"object"}}),json!(["definition"])),
        ...
        tool("processor_install","Install and pin a processor version in a fresh instance; current pointer updates cannot change it.",json!({"processor_id":{"type":"string"},"version":{"type":"string"}}),json!(["processor_id"]))
    ])
}
