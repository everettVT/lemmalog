...
| I7. Failure is bounded and uncertainty is preserved | Worker tests for timeout, malformed response, transport failure, combined completion size and acknowledgement schema | No hidden provider/MCP retry, automatic reclaim or fabricated completion; retain completed provider text when settlement fails, reject malformed acknowledgements and name the next inspection action |
